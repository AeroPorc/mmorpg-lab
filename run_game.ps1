...
Get-Process broker, gatekeeper, dedicated_server, orchestrator, client -ErrorAction SilentlyContinue | Stop-Process -Force
Start-Sleep -Milliseconds 500

Write-Host 'Building...' -ForegroundColor Cyan
cargo build -p broker -p gatekeeper -p orchestrator -p dedicated_server -p client
...
Write-Host 'Launching orchestrator (it spawns + supervises shard 0)...' -ForegroundColor Cyan
Start-Component -Title 'game-orchestrator' -Cmd "`$env:BROKER_ADDR='$broker'; `$env:SHARD_COUNT='1'; `$env:ARENA_WIDTH='900'; `$env:ARENA_HEIGHT='600'; `$env:QUAD_DEPTH='0'; cargo run -p orchestrator"
